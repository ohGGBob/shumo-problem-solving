...
+ 数据清单模板（铁律四）+ 论文模板（LaTeX 国赛 / DOCX 美赛）。
...
生成结构:
    <题名>/
      data/           原始数据
      src/            q1.py q2.py ... + export_results.py (+ gen_docx_template.py 若 --template mcm/both)
      out/            results.json(空模板) + 图表(png/svg,>=300dpi)
      report/         main.tex (国赛LaTeX) / main.docx (美赛DOCX) + figures/ + data_inventory.md(数据清单模板)
...
- `report/` 论文 `main.tex`/`main.docx`、图表 `figures/`、数据清单 `data_inventory.md`
...
# LaTeX 模板（国赛 CUMCM，中文，GB/T 7714）；<占位> 为待填项，成文时用 results.json 实际值替换
LATEX_TEMPLATE = r"""
...
# DOCX 模板说明（美赛 MCM/ICM，英文，25 页硬上限）
...
def main():
    ap = argparse.ArgumentParser(description="生成数模解题标准目录骨架 + 论文模板 + 数据清单模板")
    ...
    ap.add_argument("--template", choices=["cumcm", "mcm", "both"], default="both",
                    help="生成哪种论文模板：cumcm(LaTeX 国赛)、mcm(DOCX 美赛)、both(默认)")
    args = ap.parse_args()
    ...
    write_file(os.path.join(root, "report", "data_inventory.md"), DATA_INVENTORY_TEMPLATE)
    ...
    if args.template in ("mcm", "both"):
        # 美赛模板说明单独落盘，避免与 README 混在一起
        write_file(os.path.join(report_dir, "docx_template_note.md"), DOCX_TEMPLATE_NOTE)
    ...
    print("        data/ src/ out/ report/(figures+data_inventory.md)")
    if args.template in ("cumcm", "both"):
