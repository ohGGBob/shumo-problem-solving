...
> 场景：时间极紧（如只剩数小时）、团队某环节卡死，且**用户显式点名「紧急模式」**时进入——**由 skill 自主拍板走完全流程，但每个决策留痕、每个阶段有产物、交稿前强制收口**。
...
- **触发口令**（须**显式点名**，任一即可）：「紧急模式」 /「时间不够了，进紧急模式帮我走完」 / 显式指定 `emergency_run.py`。
- **⚠️ 不触发（重要）**：「全自动」「直接完成」**不**触发紧急模式——它们只表示"方案给全后连续执行、不再逐步停下汇报"，三类决策仍须逐项给多方案供用户选择（见 SKILL.md「强制确认门禁」）。**只有明说「紧急模式」才解锁自主决策**，门禁不可因措辞绕过。
