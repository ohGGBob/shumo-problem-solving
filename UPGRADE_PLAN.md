...
| 修复运行兼容（v1.11.1） | abstract_check.py 在 Python 3.11 下 f-string 表达式内带转义引号 → SyntaxError，`--help` 退出 1，摘要国一化检查不可用 | 把含双引号的提示语抽成局部变量再进 f-string；skill_audit A 项恢复 24/24 全绿 |
| **实测复盘回灌（v1.12.0，一次 2024A「板凳龙」全流程复算 + 20 余个子代理并行）** | ① 子代理并行解题无方法论沉淀，反复出现"空手耗尽 / 重复造轮子"；② 题目自带 `resultN.xlsx` 模板时**行列位写错**——数值全对、结构全错、按标签读数对象整体错位一个，且自检因复用同一套错误约定而"全绿"；③ **判据选错**（端点快照 / 对跳变量插值）使答案系统性偏移却查不出来 | ① 新增 `references/subagent-orchestration.md`：**先自己写好并冒烟测试核心模块再派发**、提示词八要素、**必须有一条独立复核分支（本次实测据此抓出 3 个交付文件的行列位缺陷）**、失败模式与处置、并行度与沙箱限制、汇总纪律（"分歧先问是不是同一个量"）；② 新增 `scripts/xlsx_deliverable.py`（`probe` 看模板契约 / `check` 体检行列位，空模板也能推断）；③ `execution-gotchas.md` 增三节（Excel 行列位错位、**沙箱限制清单**、**"判据选择比数值精度更致命"**）；④ `cases-2024.md` 的 2024A 章节按复算**整段重写**（原版盘入方向写反、Q3/Q4 口径与真题不符、缺 Q5；新版含五问真实口径、已核实几何事实、逐问实测答案与陷阱）；SKILL 路由表新增"子代理并行与交叉验证"行、脚本表挂接新工具、交稿收口新增第 7 条"交付物格式"；refs 54→55、scripts 24→25 |

## 二、建议后续（按优先级，赛前窗口内能做的排前面）

### P0 · 赛前 8 天（9/10 开赛前）
- [x] **环境体检**：在建模环境跑 `check_env.py`，缺库立刻装、版本锁进 `requirements.txt`。
- [x] **一键 smoke**：用 `init_project.py` 建个空项目跑 `prize_gate.py`，确认 25 个脚本在新机器全链通。
